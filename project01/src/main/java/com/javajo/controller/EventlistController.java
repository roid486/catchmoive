...
public class EventlistController {
	...
	public ModelAndView mlist(@RequestParam(value="pageNUM", defaultValue="1") int pageNUM, HttpServletRequest request)
	{
		...
		if(start > pageGroup)
		{
			pageNUM2 += "<a href='event.com?pageNUM="+(start-1)+"'>이전</a> ";
		}
		for(int i = start; i <= end; i++)
		{
			pageNUM2 += "<a href='event.com?pageNUM="+i+"'>"+i+"</a> ";
			if(i >= totalpage)
			{
				break;
			}
		...
		if(end < totalpage)
		{
			pageNUM2 += "<a href='event.com?pageNUM="+(end+1)+"'>다음</a> ";
		}
