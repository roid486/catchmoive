...
public class ClistController {
	...
	public ModelAndView blist(@RequestParam(value="node", required=false) String node,@RequestParam(value="pageNUM", defaultValue="1") int pageNUM, HttpServletRequest request)
	{
		int totalRecode = dao.totalRecode(this.key);
		int totalpage=1;
	...
	{
		this.key = key;
		int totalRecode = dao.totalRecode(key);
		int totalpage=1;
		if(totalRecode % pageSize != 0)
		{
			totalpage = (int)Math.ceil(totalRecode/pageSize)+1;
