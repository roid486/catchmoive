...
public class ReplyController {
	...
	public ModelAndView list(@RequestParam int b_number, HttpSession session) {
		...
		// String se_id=(String)session.getAttribute("se_id");
		ModelAndView mav = new ModelAndView();
		String se_id = (String) session.getAttribute("se_id");//로그인 자
		List<ReplyVo> items = dao.listRe(b_number);
		List<BoardVo> item = dao1.listBRe(b_number);
		String org_id="";
		for(int i = 0; i < item.size(); i++)
		{
			org_id = item.get(i).getC_id();
		}
		for (ReplyVo vo : items) {
				...
				if (vo.getRe_secretreply().equals("y")) {
					if (se_id == null  ) { // 비로그인 상태면 비밀 댓글로 처리
						vo.setRe_content("비밀 댓글입니다.");
					} else { // 로그인 상태일 경우
						//String writer = vo.get(); // 게시물 작성자 저장
						...
						if(!se_id.equals(org_id) && !se_id.equals(replyer)) {
	                        vo.setRe_content("비밀 댓글입니다.");
						}
				}
			}
