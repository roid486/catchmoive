...
<<<<<<< HEAD
		<center>
			...
			<span id="s"> <select id="select_sort">
					<option value="m_boxoffice">예매율순</option>
					<option value="m_score">평점순</option>
			</select>
				<button
					class="w3-btn w3-white w3-border w3-border-blue w3-round-large"
					id="btn_go">go</button>
			</span>
			<table cellspacing="5">
				<tr>
					<c:forEach var="l" items="${l }" varStatus="status">
						<td
							...
							</div> <a href="detailMovie.com?m_number=${l.m_number }"> <img
								src="resources/upload/${l.m_image }" width="220" height="320"><br></a>

							<a href="detailMovie.com?m_number=${l.m_number }">
								<div style="text-align: center; font-size: 12pt;">

									<strong><label id="div_name">${l.m_name }</label></strong>
						</a>
						</div>

							<div>
								<center>
									<label id="side_lab"><f:formatDate
											value="${l.m_opendate }" pattern="yyyy.MM.dd"></f:formatDate>
										개봉&nbsp;|&nbsp;예매율:&nbsp;${l.m_boxoffice }%</label> <br> <label
										class="side_lab">평점 : ${l.m_score }</label>
								</center>
							</div>
