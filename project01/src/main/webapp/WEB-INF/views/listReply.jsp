...
<body>
	...
	<table>
		<c:forEach var="row" items="${listRe }">
			<tr>
				<td>${row.re_number }&nbsp&nbsp&nbsp${row.c_name}&nbsp&nbsp&nbsp${row.re_regdate
					}</td>
				<td>내용:${row.re_content}(비밀여부:${row.re_secretreply})</td>
				<!-- 본인 댓글만 수정버튼 생성되도록 처리 -->
				<td><c:if test="${sessionScope.se_id == row.c_id}">
						<input type="button" id="btnModify" value="수정&삭제"
							onclick="showReplyModify('${row.re_number}')">
					</c:if></td>
			</tr>
